...
| **7660** | ISUP/EHome TCP | Публичный — открыть если используется ISUP подключение Hikvision |
| 7171 | MySQL | Только внутри сервера |
| 7272 | Backend API | Только внутри сервера |
| 7373 | Frontend | Только внутри сервера |

> Открывать наружу порт **7474** (обязательно) и **7660** (если используется ISUP/EHome). Остальные — только для отладки.
