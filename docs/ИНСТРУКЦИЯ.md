...
### Обычное обновление кода (без потери данных)

Используй когда вышло обновление системы. **Данные сотрудников, посещаемость, инвентарь — всё сохраняется.**

```bash
cd /home/ubuntu/repos/HR

# Шаг 1: Получить новый код с GitHub
# Что делает: скачивает все изменения которые были сделаны в коде
git pull

# Шаг 2: Пересобрать и перезапустить контейнеры с новым кодом
# Что делает: компилирует новый TypeScript/Next.js код и перезапускает сервисы
# Флаг --build = пересобрать образы (применить изменения кода)
# Флаг -d = запустить в фоновом режиме
docker compose up -d --build

# Шаг 3 (только если менялась схема БД): применить изменения схемы
# Что делает: синхронизирует структуру таблиц БД с новой схемой Prisma
# ВАЖНО: НЕ удаляет данные, только добавляет/меняет колонки
docker compose exec backend npx prisma db push
```

> После этих трёх команд система работает с новым кодом. Данные не тронуты.

---

### Полный переимпорт данных из СКУД (с обнулением БД)

Используй когда нужно загрузить свежий `skud.sql`. **Удаляет все данные и загружает заново.**

```bash
cd /home/ubuntu/repos/HR

# Шаг 1: Загрузить свежий skud.sql на сервер (с локального компа)
# Выполнять на ЛОКАЛЬНОМ компьютере, не на сервере:
# scp /c/repos/HR/skud.sql ubuntu@<IP_СЕРВЕРА>:/home/ubuntu/repos/HR/skud.sql

# Шаг 2: Сбросить базу данных и применить схему заново
# Что делает: удаляет ВСЕ таблицы и создаёт их заново по schema.prisma
# ВНИМАНИЕ: все данные (сотрудники, посещаемость, инвентарь) будут удалены!
docker compose exec backend npx prisma db push --force-reset

# Шаг 3: Импортировать данные из skud.sql
# Что делает: читает skud.sql, создаёт компании/отделы/должности/сотрудников,
#             импортирует события посещаемости, создаёт пользователей системы
# Время выполнения: 2-5 минут
docker compose exec backend node prisma/import-skud.js

# Шаг 4: Разложить фото сотрудников по папкам
# Что делает: берёт фото из storage/skud-photos/ и раскладывает их
#             в storage/companies/{Компания}/employees/{Сотрудник}/
docker compose exec backend node prisma/organize-photos.js
```

> После импорта логины и пароли пользователей сбрасываются на значения по умолчанию (`password`).
> Не забудь сменить пароли через Админ-панель!
